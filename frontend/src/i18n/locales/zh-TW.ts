export default {
  ...
  printers: {
    ...
    modal: {
      ...
      serialLabelOptional: '序號（選填）',
      serialU1Placeholder: '留空則依 IP 位址自動產生',
      serialU1Help: '僅作為這台印表機的唯一標籤，U1 沒有序號。',
      moonrakerToken: 'Moonraker API 權杖（選填）',
      moonrakerTokenPlaceholder: '僅當你的 Moonraker 需要時填寫',
      modelOptional: '型號（可選）',
      saveChanges: '儲存更改',
    },
    // Skip objects
  ...
  diagnostic: {
    ...
    check: {
      ...
      moonraker: {
        title: '印表機 API（Moonraker）',
        pass: '印表機已回應，Bambuddy 可以讀取狀態並送出指令。',
        fail: '該位址上的印表機沒有回應（{{error}}）。請確認它已開機、與 Bambuddy 位於同一網路，並且能透過這裡設定的位址連線。',
      },
      snapmaker_identity: {
        title: 'Snapmaker 韌體',
        pass: '已辨識為 Snapmaker {{machine_type}}。',
        pass_unknown: '已辨識為 Snapmaker 韌體。',
        warn: '印表機有回應，但未表明自己是 Snapmaker，通常代表它執行的是社群韌體。狀態顯示與基本控制應該仍可運作，部分 U1 專屬功能可能無法使用。',
        skip: '未檢查 —— 無法連線到印表機。',
      },
      klipper_ready: {
        title: '韌體就緒（Klipper）',
        pass: '韌體已就緒，可以接收指令。',
        fail: '韌體回報的狀態是「{{state}}」，並非就緒。它已停機，重新啟動前會拒絕所有指令 —— 請在印表機螢幕上執行 FIRMWARE_RESTART，或將印表機關機再開機。',
        fail_error: '無法讀取韌體狀態（{{error}}）。印表機本身有回應，但韌體沒有 —— 請重新啟動後再次執行檢查。',
        skip: '未檢查 —— 無法連線到印表機。',
      },
    },
  },
