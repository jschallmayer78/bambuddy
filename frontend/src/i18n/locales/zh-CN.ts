export default {
  ...
  printers: {
    ...
    modal: {
      ...
      serialLabelOptional: '序列号（可选）',
      serialU1Placeholder: '留空则根据 IP 地址生成',
      serialU1Help: '仅作为这台打印机的唯一标识，U1 没有序列号。',
      moonrakerToken: 'Moonraker API 令牌（可选）',
      moonrakerTokenPlaceholder: '仅当您的 Moonraker 需要时填写',
      modelOptional: '型号（可选）',
      saveChanges: '保存更改',
    },
    // Skip objects
  ...
  diagnostic: {
    ...
    check: {
      ...
      moonraker: {
        title: '打印机 API（Moonraker）',
        pass: '打印机已响应，Bambuddy 可以读取状态并发送指令。',
        fail: '该地址上的打印机没有响应（{{error}}）。请确认它已开机、与 Bambuddy 处于同一网络，并且可以通过这里配置的地址访问。',
      },
      snapmaker_identity: {
        title: 'Snapmaker 固件',
        pass: '已识别为 Snapmaker {{machine_type}}。',
        pass_unknown: '已识别为 Snapmaker 固件。',
        warn: '打印机有响应，但未表明自己是 Snapmaker，通常说明它运行的是社区固件。状态显示和基本控制应该仍可使用，部分 U1 专有功能可能无法使用。',
        skip: '未检查 —— 无法连接到打印机。',
      },
      klipper_ready: {
        title: '固件就绪（Klipper）',
        pass: '固件已就绪，可以接收指令。',
        fail: '固件报告的状态是“{{state}}”，并未就绪。它已停机，在重启之前会拒绝所有指令 —— 请在打印机屏幕上执行 FIRMWARE_RESTART，或给打印机断电重启。',
        fail_error: '无法读取固件状态（{{error}}）。打印机本身有响应，但固件没有 —— 请重启后再次运行检查。',
        skip: '未检查 —— 无法连接到打印机。',
      },
    },
  },
