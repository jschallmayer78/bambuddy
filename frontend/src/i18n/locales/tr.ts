export default {
  ...
  printers: {
    ...
    addPreflight: {
      ...
      unreachable: 'Yazıcı bu adreste yanıt vermedi.',
    },
    ...
    modal: {
      ...
      serialLabelOptional: 'Seri Numarası (isteğe bağlı)',
      serialU1Placeholder: 'IP adresinden türetmek için boş bırakın',
      serialU1Help: "Bu yazıcı için yalnızca benzersiz bir etiket — U1'in seri numarası yoktur.",
      moonrakerToken: 'Moonraker API Belirteci (isteğe bağlı)',
      moonrakerTokenPlaceholder: 'Yalnızca Moonraker gerektiriyorsa',
  ...
  diagnostic: {
    ...
    check: {
      ...
      moonraker: {
        title: 'Yazıcı API’si (Moonraker)',
        pass: 'Yazıcı yanıt verdi — Bambuddy durumunu okuyabilir ve komut gönderebilir.',
        fail: 'Bu adresteki yazıcıdan yanıt yok ({{error}}). Açık olduğundan, aynı ağda bulunduğundan ve burada girilen adresten erişilebildiğinden emin olun.',
      },
      snapmaker_identity: {
        title: 'Snapmaker donanım yazılımı',
        pass: 'Snapmaker {{machine_type}} olarak tanımlandı.',
        pass_unknown: 'Snapmaker donanım yazılımı olarak tanımlandı.',
        warn: 'Yazıcı yanıt verdi ancak kendini Snapmaker olarak tanıtmıyor — genellikle topluluk yazılımı işaretidir. Durum bilgisi ve temel denetim çalışmaya devam etmeli; U1’e özgü bazı özellikler çalışmayabilir.',
        skip: 'Denetlenmedi — yazıcıya ulaşılamadı.',
      },
      klipper_ready: {
        title: 'Donanım yazılımı hazır (Klipper)',
        pass: 'Donanım yazılımı hazır ve komutları kabul ediyor.',
        fail: 'Donanım yazılımı hazır yerine “{{state}}” bildiriyor. Durmuş durumda ve yeniden başlatılana kadar her komutu reddedecek — yazıcının ekranından FIRMWARE_RESTART çalıştırın veya yazıcıyı kapatıp açın.',
        fail_error: 'Donanım yazılımının durumu okunamadı ({{error}}). Yazıcı yanıt verdi ama donanım yazılımı vermedi — yeniden başlatıp denetimi tekrar çalıştırın.',
        skip: 'Denetlenmedi — yazıcıya ulaşılamadı.',
      },
    },
  },
