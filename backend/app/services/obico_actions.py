...
async def _pause_print(printer_id: int) -> None:
    from backend.app.services.printer_drivers.base import call_driver
    from backend.app.services.printer_manager import printer_manager

    client = printer_manager.get_client(printer_id)
    if not client:
        logger.warning("Obico pause: printer %s is not connected", printer_id)
        return
    # await-tolerant: pausing an HTTP-based printer is a request, not a publish,
    # and a coroutine left unawaited here would mean the print never paused.
    if not await call_driver(client, "pause_print"):
